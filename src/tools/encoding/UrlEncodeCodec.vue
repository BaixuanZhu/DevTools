  ...
  <div>
    ...
    <ResponsiveWorkspace mode="horizontal">
      <template #input>
        <!-- 输入区 -->
        <div class="mb-3">
          <label class="block text-[0.8125rem] text-muted-foreground font-medium mb-1">输入</label>
          <textarea
            v-model="input"
        ...
        <div v-if="urlParsed" class="border border-border rounded-md p-4 bg-card">
          <div class="text-[0.875rem] font-semibold text-primary mb-3">URL 解析</div>
          <div class="flex flex-col gap-2">
            <div class="flex items-start gap-2">
              <span class="text-[0.8125rem] text-muted-foreground shrink-0 w-20">协议</span>
              <code class="flex-1 font-mono text-[0.8125rem] break-all text-foreground">{{ urlParsed.protocol }}</code>
              <CopyButton :text="urlParsed.protocol" />
            </div>
            <div class="flex items-start gap-2">
              <span class="text-[0.8125rem] text-muted-foreground shrink-0 w-20">主机</span>
              <code class="flex-1 font-mono text-[0.8125rem] break-all text-foreground">{{ urlParsed.host }}</code>
              <CopyButton :text="urlParsed.host" />
            </div>
            <div v-if="urlParsed.port" class="flex items-start gap-2">
              <span class="text-[0.8125rem] text-muted-foreground shrink-0 w-20">端口</span>
              <code class="flex-1 font-mono text-[0.8125rem] break-all text-foreground">{{ urlParsed.port }}</code>
              <CopyButton :text="urlParsed.port" />
            </div>
            <div class="flex items-start gap-2">
              <span class="text-[0.8125rem] text-muted-foreground shrink-0 w-20">路径</span>
              <code class="flex-1 font-mono text-[0.8125rem] break-all text-foreground">{{ urlParsed.pathname }}</code>
              <CopyButton :text="urlParsed.pathname" />
            </div>
            <div v-if="urlParsed.searchParams.length > 0" class="flex items-start gap-2">
              <span class="text-[0.8125rem] text-muted-foreground shrink-0 w-20">查询参数</span>
              <div class="flex-1 flex flex-col gap-1">
                <div v-for="param in urlParsed.searchParams" :key="param.key" class="flex items-start gap-2">
                  <code class="font-mono text-[0.8125rem] text-primary">{{ param.key }}</code>
                  <span class="text-muted-foreground text-[0.8125rem]">=</span>
                  <code class="flex-1 font-mono text-[0.8125rem] break-all text-foreground">{{ param.value }}</code>
                  <CopyButton :text="`${param.key}=${param.value}`" />
                </div>
              </div>
            </div>
            <div v-if="urlParsed.hash" class="flex items-start gap-2">
              <span class="text-[0.8125rem] text-muted-foreground shrink-0 w-20">哈希</span>
              <code class="flex-1 font-mono text-[0.8125rem] break-all text-foreground">{{ urlParsed.hash }}</code>
      ...
      <template #output>
        ...
        <template v-if="currentAction === 'encode'">
          <div class="border border-border rounded-md p-4 bg-card">
            <div class="text-[0.875rem] font-semibold text-primary mb-3">编码结果</div>

            <div class="mb-3">
              <div class="flex items-baseline gap-2 mb-1.5">
                <span class="text-[0.8125rem] font-semibold text-primary font-mono">encodeURIComponent</span>
                <span class="text-[0.6875rem] text-muted-foreground">编码 :/?&amp;=# 等 URL 结构字符，适用于编码单个查询参数值</span>
              </div>
              <div class="grid grid-cols-[1fr_auto] gap-2 items-start">
                <code class="font-mono text-[0.8125rem] break-all text-foreground min-w-0 bg-accent p-2 rounded-sm">{{ encodeComponentResult || '—' }}</code>
            ...
            <div class="mb-3">
              <div class="flex items-baseline gap-2 mb-1.5">
                <span class="text-[0.8125rem] font-semibold text-primary font-mono">encodeURI</span>
                <span class="text-[0.6875rem] text-muted-foreground">保留 URL 结构字符（:/?&amp;=#），适用于编码完整 URL</span>
              </div>
              <div class="grid grid-cols-[1fr_auto] gap-2 items-start">
                <code class="font-mono text-[0.8125rem] break-all text-foreground min-w-0 bg-accent p-2 rounded-sm">{{ encodeFullResult || '—' }}</code>
            ...
            <div v-if="diffResult" class="border-t border-border pt-3 mt-3">
              <div class="text-[0.8125rem] font-semibold text-primary mb-2">差异对照</div>
              <div class="grid grid-cols-[auto_1fr] gap-2 items-start mb-1.5">
                <span class="text-[0.6875rem] text-muted-foreground shrink-0 w-[7.5rem] pt-0.5">encodeURIComponent</span>
              ...
              <div class="grid grid-cols-[auto_1fr] gap-2 items-start mb-2">
                <span class="text-[0.6875rem] text-muted-foreground shrink-0 w-[7.5rem] pt-0.5">encodeURI</span>
                <code class="font-mono text-[0.8125rem] break-all min-w-0" v-html="diffResult.fullHtml"></code>
              </div>
              <div class="text-[0.6875rem] text-muted-foreground leading-relaxed">
        ...
        <template v-if="currentAction === 'decode'">
          <div class="border border-border rounded-md p-4 bg-card">
            <div class="text-[0.875rem] font-semibold text-primary mb-3">解码结果</div>

            <div class="mb-3">
              <div class="flex items-baseline gap-2 mb-1.5">
                <span class="text-[0.8125rem] font-semibold text-primary font-mono">decodeURIComponent</span>
                <span class="text-[0.6875rem] text-muted-foreground">组件级解码</span>
              </div>
              ...
              <div v-else class="grid grid-cols-[1fr_auto] gap-2 items-start">
                <code class="font-mono text-[0.8125rem] break-all text-foreground min-w-0 bg-accent p-2 rounded-sm">{{ decodeComponentResult || '—' }}</code>
            ...
            <div>
              <div class="flex items-baseline gap-2 mb-1.5">
                <span class="text-[0.8125rem] font-semibold text-primary font-mono">decodeURI</span>
                <span class="text-[0.6875rem] text-muted-foreground">完整 URL 级解码</span>
              </div>
              <div v-if="decodeFullError" class="text-error text-[0.8125rem]">{{ decodeFullError }}</div>
              <div v-else class="grid grid-cols-[1fr_auto] gap-2 items-start">
                <code class="font-mono text-[0.8125rem] break-all text-foreground min-w-0 bg-accent p-2 rounded-sm">{{ decodeFullResult || '—' }}</code>
        ...
        <div v-if="!currentAction" class="border border-border rounded-md p-4 bg-card">
          <p class="text-muted-foreground text-[0.8125rem] m-0 text-center">点击「编码」或「解码」按钮查看结果</p>
        </div>
      </template>
    </ResponsiveWorkspace>
  </div>
