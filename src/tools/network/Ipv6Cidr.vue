...
<template>
  <div class="w-full max-w-3xl">
    ...
    <div class="mb-5 px-4 py-3 border border-border rounded-sm bg-card text-[0.8125rem] text-muted-foreground leading-relaxed">
      <p class="m-0 mb-2">
        <strong class="text-foreground">IPv6 CIDR</strong> 用
        <code class="px-1 py-0.5 bg-background rounded-sm font-mono text-[0.75rem] text-primary">地址 / 前缀长度</code>
        表示子网，前缀长度范围 0–128。与 IPv4 不同，IPv6 没有广播地址，地址空间极大（/64 子网就有约 1.8×10¹⁹ 个地址）。
      </p>
      <p class="m-0">
        例如 <code class="px-1 py-0.5 bg-background rounded-sm font-mono text-[0.75rem] text-primary">2001:db8::/32</code>
    ...
    <div class="flex items-start gap-3 mb-3">
      <div class="flex-1">
        <label class="block text-[0.8125rem] text-muted-foreground font-medium mb-1">IPv6 CIDR 地址</label>
        <input
          v-model="cidrInput"
          type="text"
    ...
    <div v-if="subnetInfo" class="flex flex-col gap-4">
      ...
      <div class="grid grid-cols-2 gap-2 max-sm:grid-cols-1">
        <div class="flex flex-col gap-1 px-4 py-2.5 border border-border rounded-sm bg-card">
          <span class="text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide">网络地址（压缩）</span>
          <div class="flex items-center gap-2">
            <span class="text-sm text-foreground font-mono break-all">{{ subnetInfo.networkAddressCompressed }}</span>
            <CopyButton :text="subnetInfo.networkAddressCompressed" />
          </div>
        </div>

        <div class="flex flex-col gap-1 px-4 py-2.5 border border-border rounded-sm bg-card">
          <span class="text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide">网络地址（展开）</span>
          <div class="flex items-center gap-2">
            <span class="text-sm text-foreground font-mono break-all">{{ subnetInfo.networkAddressExpanded }}</span>
            <CopyButton :text="subnetInfo.networkAddressExpanded" />
          </div>
        </div>

        <div class="flex flex-col gap-1 px-4 py-2.5 border border-border rounded-sm bg-card">
          <span class="text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide">前缀长度</span>
          <span class="text-sm text-foreground font-mono">/{{ subnetInfo.prefix }}</span>
        </div>

        <div class="flex flex-col gap-1 px-4 py-2.5 border border-border rounded-sm bg-card">
          <span class="text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide">地址总数</span>
          <span class="text-sm text-foreground font-mono break-all">{{ subnetInfo.totalAddresses }}</span>
        </div>

        <div class="flex flex-col gap-1 px-4 py-2.5 border border-border rounded-sm bg-card">
          <span class="text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide">范围首地址</span>
          <div class="flex items-center gap-2">
            <span class="text-sm text-foreground font-mono break-all">{{ subnetInfo.firstAddressCompressed }}</span>
            <CopyButton :text="subnetInfo.firstAddressCompressed" />
          </div>
        </div>

        <div class="flex flex-col gap-1 px-4 py-2.5 border border-border rounded-sm bg-card">
          <span class="text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide">范围末地址</span>
          <div class="flex items-center gap-2">
            <span class="text-sm text-foreground font-mono break-all">{{ subnetInfo.lastAddressCompressed }}</span>
        ...
        <div class="flex flex-col gap-1 px-4 py-2.5 border border-border rounded-sm bg-card col-span-2 max-sm:col-span-1">
          <span class="text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide">地址类型</span>
          <span class="text-sm text-foreground">
            <strong>{{ subnetInfo.type.label }}</strong>
            <span class="text-muted-foreground"> · {{ subnetInfo.type.description }}</span>
          </span>
        </div>
      </div>
      ...
      <div class="border-t border-border pt-4">
        <h3 class="text-[0.8125rem] text-muted-foreground font-medium">IPv6 前缀长度参考</h3>
        <div class="pt-2">
            ...
            <table class="w-full text-[0.8125rem]">
              <thead>
                <tr class="border-b border-border text-left">
                  ...
                  <th class="px-3 py-2 text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide">典型用途</th>
                  <th class="px-3 py-2 text-[0.6875rem] font-semibold text-muted-foreground uppercase tracking-wide text-right">地址数</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  ...
                  :class="[
                    ...
                    row.prefix === subnetInfo.prefix ? 'bg-primary/5' : 'hover:bg-accent',
                  ]"
                >
                  <td class="px-3 py-1.5 font-mono font-semibold text-primary">/{{ row.prefix }}</td>
                  <td class="px-3 py-1.5 text-foreground">{{ row.usage }}</td>
                  <td class="px-3 py-1.5 text-right font-mono text-foreground">{{ row.total }}</td>
                </tr>
              </tbody>
            </table>
          </div>
