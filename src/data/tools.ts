...
export const tools: ToolMeta[] = [
  ...
  },
  {
    ...
    seoDescription: '在线 MySQL 配置文件生成器，按内存、磁盘、使用场景与并发实时生成可直接使用的 my.cnf，支持 5.7/8.0/8.4 版本联动过滤与废弃参数改名提示（tx_isolation、redo log 等矩阵）、单机与主从模式（server_id 随机种子、GTID）、参数中文说明与官方文档溯源，附带 OS 内核参数建议，可一键复制下载，纯浏览器端运算数据不上传。',
    category: '开发与运维',
    icon: '🐬',
  ...
  },
  {
    ...
    seoDescription: '免费在线 ICO 图标制作工具：导入图片后按 1:1 裁切创作主体（或跳过裁切直接生成），多选 16 / 32 / 48 / 64 / 128 / 256 尺寸一键封装多尺寸 favicon .ico 文件，逐尺寸真实像素预览并可单独导出 PNG；同时支持解析 ICO / CUR 图标文件，查看内嵌条目并把 PNG / BMP 图像提取为 PNG，纯浏览器端本地处理，图片绝不上传。',
    category: '前端与媒体',
    icon: '🧩',
