...
export const tools: ToolMeta[] = [
  ...
  },
  {
    ...
    relatedToolIds: ['symmetric-crypto', 'jwt-parser', 'base64', 'bcrypt'],
  },
  {
    id: 'bcrypt',
    name: 'BCrypt 密码哈希',
    title: 'BCrypt 密码哈希 - 在线生成与校验、cost 耗时对照 - DevTools',
    description: '生成带随机盐的 bcrypt 密码哈希，支持多种前缀哈希在线校验与解析，自动提示 72 字节截断',
    seoDescription: '免费在线 BCrypt 密码哈希工具，输入密码自定义 cost 因子一键生成带随机盐的标准 bcrypt 哈希，支持 $2a$/$2b$/$2y$ 各前缀哈希在线校验比对与版本、盐值解析，自动提示 72 字节截断风险，用户表密码存储与登录接口调试必备，纯浏览器端运算密码绝不上传。',
    category: '加密与安全',
    icon: '🔐',
    path: '/crypto/bcrypt',
    keywords: ['bcrypt 在线', 'bcrypt 生成', 'bcrypt 校验', 'bcrypt cost', '密码哈希', 'bcrypt hash', 'bcrypt 解析', '密码存储加密'],
    relatedToolIds: ['argon2', 'pbkdf2', 'hash-generator'],
  },
  {
    id: 'argon2',
    name: 'Argon2 密码哈希',
    title: 'Argon2 密码哈希 - 在线生成与校验、argon2id 调参 - DevTools',
    description: '生成带随机盐的 argon2id/i/d 密码哈希，自定义内存、迭代与并行度，在线校验 PHC 哈希并解析参数',
    seoDescription: '免费在线 Argon2 密码哈希工具，支持 argon2id、argon2i、argon2d 三种类型，自定义内存、迭代次数与并行度参数生成标准 PHC 格式哈希，粘贴既有哈希即可在线校验密码是否匹配并解析参数，新项目密码存储选型与迁移数据核对必备，纯浏览器端运算密码绝不上传。',
    category: '加密与安全',
    icon: '🧂',
    path: '/crypto/argon2',
    keywords: ['argon2 在线', 'argon2 生成', 'argon2 校验', 'argon2id', '密码哈希', 'phc 格式', 'argon2 hash', '密码存储加密'],
    relatedToolIds: ['bcrypt', 'pbkdf2', 'hash-generator'],
  },
  {
    id: 'pbkdf2',
    name: 'PBKDF2 密钥派生',
    title: 'PBKDF2 密钥派生 - 在线生成与 Django 哈希校验 - DevTools',
    description: '自定义盐、迭代与 PRF 派生密钥（hex/Base64 输出），在线校验 Django pbkdf2_sha256 哈希并解析参数',
    seoDescription: '免费在线 PBKDF2 密钥派生工具，自定义盐、迭代次数与 PRF 哈希算法派生十六进制或 Base64 密钥，支持 Django pbkdf2_sha256 格式哈希在线校验与参数解析，跨语言对接联调与迁移数据核对必备，纯浏览器端运算密码与密钥绝不上传。',
    category: '加密与安全',
    icon: '🧮',
    path: '/crypto/pbkdf2',
    keywords: ['pbkdf2 在线', 'pbkdf2 生成', 'django 密码哈希', 'pbkdf2_sha256', '密钥派生', 'pbkdf2 迭代', 'kdf 在线'],
    relatedToolIds: ['argon2', 'bcrypt', 'hash-generator'],
  },
  {
    id: 'random-string',
    name: '随机字符串生成',
