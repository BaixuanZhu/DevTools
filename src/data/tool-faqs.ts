...
const toolFaqs: Record<string, FaqItem[]> = {
  ...
  'image-scrambler': [
    {
      question: '混淆后的图片能当作加密使用吗？',
      answer: '可以视为一种<strong>轻度混淆</strong>，但不是现代密码学意义上的加密。它通过重排像素块的位置让图片看起来像抽象拼图；如果攻击者知道算法和参数，仍然可以还原。如需高强度保护，请使用专业加密工具。',
    },
    {
      question: '下载的混淆图片如何还原？',
      answer: '直接把下载的 PNG 重新上传到本页即可<strong>自动还原</strong>——还原所需的参数已写入图片元数据（PNG tEXt 块），无需手动记录任何参数。即便部分平台重编码清除了元数据，工具还会从文件名中兜底识别。',
    },
    {
      question: '为什么还原需要与混淆时相同的参数？',
      answer: '块级混淆是一种<strong>可逆变换</strong>，必须使用与混淆完全相同的<strong>块大小</strong>置换顺序才能对得上。工具会自动生成还原参数并写入图片元数据，正常使用流程下无需手动记录；只有外部工具产出或无元数据的混淆图才需要手动指定参数。',
    },
    {
      question: '为什么下载的文件名很长？',
      answer: '文件名里内嵌了还原所需的<strong>参数</strong>作为兜底：当社交平台或图片处理工具重编码清除了 PNG 元数据（tEXt 块）时，工具仍能从文件名识别参数并完成还原。请尽量保留完整文件名。',
    },
    {
      question: '为什么混淆后的图片体积变大了？',
      answer: '属正常现象。混淆后像素被打乱为随机分布，PNG 无损压缩失效，体积会显著增大，但<strong>不影响还原</strong>——还原后像素恢复原序，体积也随之回到正常水平。',
    },
    {
      question: '块大小应该怎么选？',
      answer: '块大小决定重排的颗粒度：<strong>小块（如 2×2）</strong>打散更细，结果更接近噪点；<strong>大块（如 128×128）</strong>置换表更小、速度更快，但结果保留明显的拼图块轮廓。可在 2、4、8、16、32、64、128 之间按需切换。',
    },
    {
      question: '可以批量处理多张图片吗？',
      answer: '当前版本一次只处理一张图片。如需批量处理，可下载结果后再次上传下一张。',
    },
  ],
  'phantom-tank': [
    {
      question: '为什么白底和黑底会看到不同的图？',
      answer: '这是<strong>透明度（Alpha）混合</strong>的效果。工具为每个像素算出一个透明度，使结果图叠在白色背景上时透出图A、叠在黑色背景上时显出图B。同一张透明 PNG 在不同背景下被浏览器或看图软件用不同背景合成，就呈现出两幅不同的画面。',
    },
    {
      question: '在哪里能看到双重显示效果？',
      answer: '任何会改变背景的地方：聊天软件的<strong>深色 / 浅色模式</strong>、不同颜色的聊天气泡、网页深浅色主题，或本工具结果区的「白底 / 黑底 / 棋盘格」背景切换。把图片发到深浅模式不同的设备上对比即可看到「一图双义」。',
    },
    {
      question: '怎么让双重效果更清晰？',
      answer: '建议<strong>图A（表图）偏亮、图B（里图）偏暗</strong>，并尽量提高两图对比度。当图A整体较亮、图B整体较暗时，合成后白底更易透出图A、黑底更易显出图B，双重效果最分明。',
    },
    {
      question: '两张图尺寸不一样可以用吗？',
      answer: '可以。工具会自动取两张图<strong>较小的宽和高</strong>作为输出尺寸，并各自从<strong>中心裁剪</strong>对齐（不缩放、保持像素清晰），因此建议两图主体尽量居中，避免边缘内容被裁掉。',
    },
    {
      question: '只有一张图能用吗？',
