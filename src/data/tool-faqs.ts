...
const toolFaqs: Record<string, FaqItem[]> = {
  ...
  'image-scrambler': [
    {
      question: '置乱后的图片能当作加密使用吗？',
      answer: '可以视为一种<strong>轻度混淆</strong>，但不是现代密码学意义上的加密。它主要改变像素位置或颜色分布，让图片看起来像噪点；如果攻击者知道算法和参数，仍然可以还原。如需高强度保护，请使用专业加密工具。',
    },
    {
      question: '为什么还原需要与置乱时相同的参数？',
      answer: '三种算法都是<strong>可逆变换</strong>，但还原必须使用与置乱完全相同的算法、迭代次数、种子等参数。本工具会在切换「置乱/还原」模式时保留当前参数，方便你直接使用。',
    },
    {
      question: 'Arnold 变换为什么要处理成正方形？',
      answer: 'Arnold 变换的数学定义要求像素矩阵为 <code>N×N</code> 方阵。本工具对非正方形图片提供两种策略：<strong>边缘外扩填充</strong>（不丢内容，输出为带边缘填充的正方形，可完整还原）和<strong>居中裁切</strong>（丢失边缘但算法更标准）。',
    },
    {
