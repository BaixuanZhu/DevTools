# FAQ 常见问题区块 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 为每个工具页面增加 FAQ（常见问题）区块，覆盖长尾关键词并降低跳出率，首批覆盖 10 个高频工具共 33 条 FAQ。

**Architecture:** FAQ 数据集中管理在独立的 `tool-faqs.ts` 文件中，由 `ToolLayout.astro` 在构建时通过 `getToolFaqs()` 查询并渲染为纯 HTML 的 `<details>/<summary>` 区块（零 JS 增量）。22 个 Vue 工具组件零改动。

**Tech Stack:** Astro 模板 + 原生 HTML `<details>/<summary>` + Tailwind CSS
...
| 文件 | 操作 | 职责 |
|------|------|------|
| `src/data/tool-faqs.ts` | 新建 | FAQ 数据类型定义 + `getToolFaqs()` 辅助函数 + 首批 10 个工具共 33 条 FAQ 数据 |
| `src/layouts/ToolLayout.astro` | 修改 | 导入 `getToolFaqs`，在 `<slot />` 之后、RelatedTools 之前渲染 FAQ 区块 |

---

### Task 1: 创建 `src/data/tool-faqs.ts` — 类型定义与辅助函数

**Files:**
- Create: `src/data/tool-faqs.ts`

- [ ] **Step 1: 创建文件，写入类型定义和辅助函数**
...
Run: `pnpm build`
Expected: 构建成功（FAQ 数据为空，不影响任何页面）

- [ ] **Step 3: Commit**

```bash
git add src/data/tool-faqs.ts
git commit -m "feat(data): 新建 tool-faqs.ts，定义 FAQ 类型与 getToolFaqs 辅助函数"
```

---

### Task 2: 填充首批 10 个工具的 FAQ 数据（33 条）

**Files:**
- Modify: `src/data/tool-faqs.ts`

- [ ] **Step 1: 将 `toolFaqs` 空对象替换为包含 10 个工具 FAQ 的完整数据**

将 `src/data/tool-faqs.ts` 中的 `const toolFaqs: Record<string, FaqItem[]> = {};` 替换为以下内容：
...
```bash
git add src/data/tool-faqs.ts
git commit -m "feat(data): 填充首批 10 个工具的 33 条 FAQ 数据"
```

---

### Task 3: 修改 ToolLayout.astro — 渲染 FAQ 区块

**Files:**
- Modify: `src/layouts/ToolLayout.astro:1` — 添加 import
- Modify: `src/layouts/ToolLayout.astro:340-346` — 在 slot 和 RelatedTools 之间插入 FAQ 区块

- [ ] **Step 1: 在 frontmatter 中添加 `getToolFaqs` 导入**

在 `ToolLayout.astro` 第 6 行的 import 语句中追加 `getToolFaqs` 导入。
...
- [ ] **Step 2: 在 frontmatter 中获取 FAQ 数据**

在第 35 行（`const relatedToolIds = toolMeta?.relatedToolIds || [];`）之后添加一行：
...
- [ ] **Step 3: 在模板中 slot 之后、RelatedTools 之前插入 FAQ 区块**

将 `ToolLayout.astro` 中第 340-346 行的这段代码：
```astro
        <slot />
...
git commit -m "feat(layout): ToolLayout 渲染 FAQ 区块，原生 details/summary 零 JS"
```
...
访问 `http://localhost:4321/encoding/base64`，检查：
1. 操作区下方显示"常见问题"区块标题
2. 4 个问题项可见，每项之间有分隔线
3. 点击问题 → 折叠/展开正常，箭头旋转 180°
4. 答案中 `<strong>` 和 `<code>` 标签正确渲染

对以下页面重复验证（检查 FAQ 存在且可交互）：
- `/encoding/url-encode`（3 条 FAQ）
- `/encoding/jwt-parser`（4 条 FAQ）
- `/format/json-formatter`（3 条 FAQ）
- `/crypto/hash-generator`（4 条 FAQ）
- `/text/uuid-generator`（3 条 FAQ）
- `/crypto/symmetric-crypto`（3 条 FAQ）
- `/media/qr-code-generator`（3 条 FAQ）
- `/datetime/cron-parser`（3 条 FAQ）
- `/network/http-status-codes`（2 条 FAQ）

- [ ] **Step 3: 验证未配置 FAQ 的工具页**

访问以下页面（未在 FAQ 数据中配置），确认不显示"常见问题"区块：
- `/network/device-info`
- `/format/json-diff`
...
在 Base64 页面：
1. Tab 键可以聚焦到每个 `<summary>` 元素
2. Enter 键可以展开/折叠
3. 屏幕阅读器能正确朗读问题文本

- [ ] **Step 5: 验证构建产物不含额外 JS**

Run: `pnpm build`
检查构建日志确认 FAQ 区块未引入额外的 JS chunk（FAQ 使用原生 HTML，不涉及 Vue 水合）。
...
| Spec 要求 | 对应 Task | 状态 |
|-----------|----------|------|
| 新建 `src/data/tool-faqs.ts` | Task 1 + 2 | ✅ |
| `FaqItem` 接口 + `getToolFaqs()` 函数 | Task 1 | ✅ |
| ToolLayout 导入并渲染 FAQ | Task 3 | ✅ |
| 原生 `<details>/<summary>` 渲染 | Task 3 | ✅ |
| `set:html` 渲染 answer | Task 3 | ✅ |
| 箭头旋转动画 `group-open:rotate-180` | Task 3 | ✅ |
| `list-none` 移除默认三角 | Task 3 | ✅ |
| FAQ 在 slot 后、RelatedTools 前 | Task 3 | ✅ |
| 未配置 FAQ 不渲染区块 | Task 3（空数组时不渲染） | ✅ |
| 10 个工具 33 条 FAQ | Task 2 | ✅ |
| 不引入新 npm 依赖 | 全部 | ✅ |
| 不修改 Vue 工具组件 | 全部 | ✅ |
| 不使用 Vue 水合 | Task 3（纯 Astro/HTML） | ✅ |
| 不删除 DisclosureSection.vue | 全部 | ✅ |

### 2. Placeholder Scan

无 TBD、TODO、"implement later"、"fill in details" 等占位符。所有代码步骤包含完整代码。

### 3. Type Consistency

- `FaqItem` 接口在 Task 1 中定义，Task 2 中使用其 `question`/`answer` 字段，Task 3 中通过 `faq.question`/`faq.answer` 消费 — 一致。
- `getToolFaqs(toolId: string)` 接受完整 toolId（如 `encoding/base64`），内部提取 slug — 与 ToolLayout 中 `toolId` prop 的格式一致。
- FAQ 数据键使用短 ID（`'base64'`、`'url-encode'` 等）— 与 `tools.ts` 中 `ToolMeta.id` 命名一致。
