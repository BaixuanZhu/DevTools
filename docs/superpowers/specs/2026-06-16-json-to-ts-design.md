...
- 对象字段 `field` 的值若为**对象**，其 interface 名 = `父对象 interface 名 + capitalize(field)`（如 `RootObject` 的字段 `meta` → `RootObjectMeta`）
- 对象字段 `field` 的值若为**对象数组**，数组元素的 interface 名 = `父对象 interface 名 + capitalize(field) + Item`（如字段 `users` 的元素 → `RootObjectUsersItem`）
- 维护「已使用类型名」集合，派生名冲突时追加数字后缀（`RootObjectUsersItem2`）
...
interface RootObject {
  users: RootObjectUsersItem[];
  count: number;
  tags: (string | number)[];
  meta: null;
}

interface RootObjectUsersItem {
  id: number;
  name: string;
  active?: boolean;
}
...
### 6.1 输入与解析
- **复用 `parseJsonSafe`**（来自 `./json-diff`，与 json-to-yaml/xml 一致）：一次调用即获得语法解析 + 行列定位的错误信息，并自带两层保护——嵌套深度硬限 **256 层**、节点总数上限 **50 万**，超限返回中文错误
...
### 6.4 递归深度与栈安全（核心）
深度保护主要复用 `parseJsonSafe` 已验证的机制，推断阶段再叠加兜底，避免重复造轮子：

1. **解析阶段硬限（复用 `parseJsonSafe`）**：嵌套超过 **256 层** 或节点超过 **50 万** 时直接返回中文错误，根本不会进入推断递归——这是 json-to-yaml/xml 已在生产使用的同款保护
2. **推断阶段兜底**：256 层以内的递归推断对 V8 调用栈是安全的；仍对推断整体 `try-catch` 捕获 `RangeError`，命中时返回友好错误（双保险，应对极端帧开销）
3. **Worker 隔离**：大输入与全部推断逻辑跑在 Web Worker，即便最坏情况栈溢出，也只影响 Worker（主线程 `onerror` 捕获并提示），不卡死页面

> 说明：相比早期设想「100 层软阈值 + 兜底 unknown」，复用 `parseJsonSafe` 的 256 层硬限更简单、更 DRY，且与同分类工具完全一致；256 层远超任何真实数据（通常 < 20 层），硬限不会误伤正常输入。
...
12. 嵌套超 256 层 / 节点超 50 万时 `parseJsonSafe` 返回中文深度错误（复用既有保护）
